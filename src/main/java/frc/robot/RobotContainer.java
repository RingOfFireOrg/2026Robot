...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        // autoChooser.addOption("PreloadTrench", new
        // PathPlannerAuto("WakePreloadDepotTrench"));
        // autoChooser.addOption("Center Preload + Depot V1", new
        // PathPlannerAuto("CenterDepotAutoV1", false));
        // autoChooser.addOption("Center Preload + Depot V2", new
        // PathPlannerAuto("CenterDepotAutoV2", false));
        autoChooser.addOption("rolling", Rollin.create(intake));
        // autoChooser.addOption("Trench Depot", new PathPlannerAuto("TrenchDepotAuto",
        // false));
        ...
        // autoChooser.addOption("robotTest", new RobotDriveForTime(drive, 1.0, 0.0,
        // 0.0, 3.0));
        autoChooser.addOption("Robot Center Preload",
                CenterPreloadRobotAuto.create(drive, turret, indexer, transfer, intake));
        autoChooser.addOption("Robot Center Preload Depot",
                CenterPreloadDepotRobotAuto.create(drive, turret, indexer, transfer, intake));
        autoChooser.addOption("ballin", Ballin.create(turret, indexer, transfer, intake));
        /*
         * autoChooser.addOption("Preload", new PathPlannerAuto("CenterPreloadAuto",
         * false));
         * autoChooser.addOption("Preload and Depot V2(use this one)", new
         * PathPlannerAuto("CenterPreloadDepotAuto", false));
         * autoChooser.addOption("TrenchTest", new PathPlannerAuto("WakeTrenchTest",
         * false));
         ...
         * autoChooser.addOption("test Preload", new PathPlannerAuto("PreloadTest",
         * false));
         */
        //// autoChooser.addOption("Limelight score(DONT USE)", new
        //// PathPlannerAuto().andThen());
    ...
    private void configureButtonBindings() {
        ...
        drive.setDefaultCommand(DriveCommands.joystickDrive(
                drive,
                () -> {
                    double maxSpeedX = (1 - driver.getLeftTriggerAxis())
                            * (standardSpeed + (1 - standardSpeed) * driver.getRightTriggerAxis());
                    return -1 * MathUtil.applyDeadband(MathUtil.clamp(driver.getLeftY(), -maxSpeedX, maxSpeedX), 0.1);
                },
                () -> {
                    double maxSpeedY = (1 - driver.getLeftTriggerAxis())
                            * (standardSpeed + (1 - standardSpeed) * driver.getRightTriggerAxis());
                    return -1 * MathUtil.applyDeadband(MathUtil.clamp(driver.getLeftX(), -maxSpeedY, maxSpeedY), 0.1);
                },
                () -> {
                    double maxSpeedTheta = (1 - driver.getLeftTriggerAxis())
                            * (turnSpeed + (1 - turnSpeed) * driver.getRightTriggerAxis());
                    return MathUtil.applyDeadband(MathUtil.clamp(-driver.getRightX(), -maxSpeedTheta, maxSpeedTheta),
                            0.1);
                }));

        driver.povLeft().whileTrue(DriveCommands.joystickDriveRobotOriented(drive, () -> 0,
                () -> -0.5 + -(driver.getRightTriggerAxis() / 2), () -> 0));
        driver.povRight().whileTrue(DriveCommands.joystickDriveRobotOriented(drive, () -> 0,
                () -> 0.5 + (driver.getRightTriggerAxis() / 2), () -> 0));
        driver.povDown().whileTrue(DriveCommands.joystickDriveRobotOriented(drive,
                () -> 0.5 + (driver.getRightTriggerAxis() / 2), () -> 0, () -> 0));
        driver.povUp().whileTrue(DriveCommands.joystickDriveRobotOriented(drive,
                () -> -0.5 + -(driver.getRightTriggerAxis() / 2), () -> 0, () -> 0));
        ...
        // driver.back().onTrue(Commands.runOnce(resetGyro,
        ...
        if (Constants.currentMode == Constants.Mode.REAL) {
            ...
            // driver.b().whileTrue(
            // DriveAim.create(drive, "limelight-tag", () -> -driver.getLeftY(), () ->
            // -driver.getLeftX()).onlyIf(() -> LimelightHelpers.getTV("limelight-tag")));
            ...
            // operator.a().whileTrue(Commands.parallel(transfer.runPercent(transfer::getFeedPercent),
            // indexer.runPercent(indexer::getReversePercent))); //Transfer and indexer out
            // operator.b().whileTrue(intake.rollersIn()); //outtake
            // operator.b().whileTrue(Commands.parallel(intake.shakeBalls(),
            // intake.rollersOut()));
            ...
            operator.povDown().whileTrue(Commands.parallel(indexer.runVelocityRpm(indexer::getReverseRpm),
            ...
            operator.rightTrigger()
                    .whileTrue(turret.runShooterRPM(turret::getDashboardTopRpm, turret::getDashboardBottomRpm));
            operator.back().onTrue(Commands.runOnce(() -> turret.zeroTurret(), turret));
            // run shooter at rpm determined by vision, run indexer when shooter is right
            // speed
            // operator.leftTrigger().whileTrue(turret.runShooterRPM(() -> -2000,
            // ()->1500));
         ...
         * // AutoBuilder.pathfindThenFollowPath(PathPlannerPath.fromPathFile("toReed"),
         * constraints));
         * // }
    ...
    public void setNamedCommands() {
         ...
         * );
         */
        // NamedCommands.registerCommand("Align Center", new AlignToReef(drive,
        // reefSide.CENTER).withTimeout(2));
        // NamedCommands.registerCommand("Align Left", new AlignToReef(drive,
        // reefSide.LEFT).withTimeout(2));
        // NamedCommands.registerCommand("Align Right", new AlignToReef(drive,
        // reefSide.RIGHT).withTimeout(2));
